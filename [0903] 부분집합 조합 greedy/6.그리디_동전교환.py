...
# 아래 3가지를 만족시키지 않는다면 그리디로 못풀어!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# 1. 각 단계에서 최적해를 찾아야 한다.
# 2. 각 단계의 결과들을 합하는 방법을 찾아야 한다.
# 3. 각 단계의 합 == 전체 문제의 합이라는 것을 증명해야 한다.
# 그리디 문제들을 어떻게 잘 해결할까? 실력을 어떻게 늘릴까? 유형을 많이 보는 수밖에...
# 규칙 찾고 조건 다 만족하고 반례도 없고 그리디라는 확신을 가졌다면?? 코드 구현은 어렵지 않다

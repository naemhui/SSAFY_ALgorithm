...
for tc in range(1, T+1):
    ...
    # 맨 처음에는 오븐의 크기(N)개 피자를 꺼내 오븐에 넣기
    for i in range(N):
        rear = (rear + 1) % qsize
        oven[rear] = i  # 피자 번호 i (우리는 0번부터, 문제에서는 1번부터)
    ...
    # 오븐 안에 피자가 남아있으면 계속 해
    while front != rear:  # 오븐 안에 피자가 있을 때까지만
        # 오븐 안에서 피자 하나 꺼내기
        front = (front + 1) % qsize
        ...
        # 치즈가 0이 됐으면 꺼내고 새로운 피자를 넣자
            ...
            # 현재 이 피자가 마지막 피자이거나
            # 마지막 피자이면 오븐이 비었기 때문에 반복문 자동 종료

    # 마지막으로 꺼낸 피자 번호 +1해서 출력력

###########강사님 풀이
# 버스를 운행하는 함수
def drive(K, N):
    # return 0 : 충전소가 제대로 배치되지 않아서 운행 불가능
    # return > 0 : 충전소가 제대로 배치되어서 충전한 횟수를 리턴

    last = 0  # 마지막으로 충전한 위치 (운행 불가능한 상황 판별)
    now = K  # 현재 버스 위치(정류장 번호), 처음에 충전된 상태로 간다고 했으니까 K만큼 미리 이동
    count = 0  # 충전 횟수

    # 버스 운행 하기
    # 1. 일단 최대로 간다(k만큼)
    # 2 -1. 충전기가 있으면 1로 가서 반복
    # 2 -2. 충전기가 없으면 앞으로 한칸 돌아오기
    # 2 -2 -1. 충전기가 있으면 1로 가서 반복
    # 2 -2 -2. 충전기가 없으면 앞으로 한칸 돌아오기
    # ...
    # 원래 위치로 돌아와 버리면 운행 불가능
    # 현재 위치가 마지막 정류장위치 이상 이면 운행 가능

    # 종점 위치 N보다 작으면 계속 버스 운행
    # N이상이면 운행 중단
    while now < N:
        # if stop[now] == 0:
        #     now -= 1
        #     if stop[now] == 0:
        #         now -= 1
        #         if stop[now]
        #

        # 현재 위치에 충전기 없으면 앞으로 한칸씩 계속 이동
        while stop[now] == 0:
            now -= 1
            # 뒤로 가다가 마지막 충전 위치로 와버리면 운행 불가
            if now == last:
                # 운행 불가, 이 이상 진행 할 필요 없으니 함수 종료
                return 0

        # 여기 코드가 실행되면 중간에 충전기를 만났다는 것이다.
        # 마지막 충전 위치 갱신
        last = now
        # 다음 위치로 이동(최대한 이동 해야 하니까 K만큼)
        now += K
        # 충전횟수 증가
        count += 1

    # 여기 코드가 실행되면 반복문이 잘 종료가 되었다 (중단없이)
    # now >= N 이라는 것 -> 제대로 도착 했다는 것
    return count


T = int(input())

for tc in range(1, T + 1):
    K, N, M = map(int, input().split())
    # K : 한번 충전시 이동 가능한 거리
    # N : 버스가 가야할 거리(정류장 개수)
    # M : 충전기 개수

    # 충전기의 위치 모음(리스트)
    charge_list = list(map(int, input().split()))

    # 정류장에 충전소가 있나 없나 여부를 나타내는 리스트
    stop = [0] * N
    # stop[i] => i번째 정류장에 충전기가 있나 없나 를 나타낸다.
    # stop[i] == 1 : i번 정류장에는 충전기가 있다.
    # stop[i] == 0 : i번 정류장에는 충전기가 없다.

    for x in charge_list:
        stop[x] = 1  # x번 정류장에는 충전소가 있다.

    answer = drive(K, N)
    print(f"#{tc} {answer}")



# 내 코드와 강사님 코드 차이
#
# 1. 접근 방식의 차이:
#    - 나: 충전소를 순차적으로 확인하며 전진하는 방식을 사용
#    - 강사님: 최대한 앞으로 이동한 후 뒤로 돌아오며 충전소를 찾는 방식을 사용
#
# 2. 충전소 정보 저장 방식:
#    - 나: 충전소 위치만을 리스트로 저장
#    - 강사님: 모든 정류장에 대해 충전소 유무를 배열로 저장. 이 방식이 더 효율적이고 오류 가능성이 낮을 듯
#
# 3. 운행 불가능 상황 판단:
#    - 나: 연속된 충전소 간 거리만을 확인했습니다.
#    - 강사님: 마지막 충전 위치를 기록하고, 이를 이용해 더 정확하게 운행 불가능 상황을 판단
#
# 4. 종점 도달 확인:
#    - 나: 종점을 충전소 리스트에 추가하여 확인
#    - 강사님: 현재 위치가 종점 이상인지 직접 확인
#
# 5. 충전 횟수 계산:
#    - 나: 충전이 필요할 때마다 카운트를 증가
#    - 강사님: 충전소를 찾을 때마다 카운트를 증가시킴
#
# 그래서 나는 문제에서 제시된 테스트 케이스는 맞았지만 나머지 내부의 추가 테스트 케이스에서 fail 뜬 듯
# 특히 운행 불가능 상황을 판단하는 로직과 충전소 정보 저장 방식의 차이가 중요한 요인. 강사님의 코드는 더 견고하고 다양한 상황을 잘 처리할 수 있음